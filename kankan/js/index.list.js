...
function createListItem(item, imgwidth) {
	...
	var price = item.price ? parseFloat(item.price).toLocaleString() : "0";
	if(heicha > 0) {
		var toppx = heicha / 2;
	...
	listItemHTML.appendFormat('<div class="li-content" id="{0}">', item.commodity_id)
		...
		.append('</div>')
		.append('<div class="suspension_price"><span>&yen;<span style="padding-left:0.4em">'+price+'</span></span></div>')
...
function getzanItems() {
	...
	zanItemHTML.append('<div id="clickzan" class="mui-popover mui-popover-action mui-popover-bottom">')
	    ...
	    .append('</li>')
	    .append('<li class="mui-table-view-cell">')
	    .appendFormat('<a id="rulesItem" style="color: #007aff">{0}</a >', TextMessage.rulesitem)
	    .append('</li>')
	    .append('</ul>')
	    .append('<ul class="mui-table-view">')
	    .append('<li class="mui-table-view-cell">')
